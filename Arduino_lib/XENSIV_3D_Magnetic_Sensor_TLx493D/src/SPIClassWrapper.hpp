    ...
    namespace tlx493d {
        ...
        class SPIClassWrapper {
            ...
            public:
                ...
                bool transfer(uint8_t *txBuffer, uint8_t txLen, uint8_t *rxBuffer, uint8_t rxLen, uint8_t readAddress) {
                    // TX Teil: Sende Daten wenn vorhanden
                    if( (txLen > 0) && (txBuffer != NULL) ) {
                    ...
                    // RX Teil: Empfange Daten wenn gewünscht
                    if( (rxLen > 0) && (rxBuffer != NULL) ) {
                        uint16_t bytesRead = 0;

                        spi->transfer(0xC0);  //auto increment read command
                        
                        // Read the remaining bytes holding the data from previous measurement
                        ...
                        // Shift all rxBuffer values one bit to the right
                        // Bit 0 of each byte becomes bit 7 of the next byte
